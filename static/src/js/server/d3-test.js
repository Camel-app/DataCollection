function draw() {
	...
	svg.append("g")
		...
		.attr("x1", function (d) {
			return getPosition(d.source).x;
		})
		.attr("y1", function (d) {
			return getPosition(d.source).y;
		})
		.attr("x2", function (d) {
			return getPosition(d.target).x;
		})
		.attr("y2", function (d) {
			return getPosition(d.target).y;
		});
	...
	svg.append("g")
		...
		.attr("stroke", function (node) {
			return !node.selected ? getColourNodeStroke(node) : "black";
		})
		.attr("stroke-width", function (node) {
			return Math.abs(node.value) * 2;
		})
