...
let rightButtonSelectorSaveButton = document.getElementById("rightButton");
rightButtonSelectorSaveButton.innerHTML += saveButton;
...
function saveCam() {
	let CAMnodes = CAM.nodes.filter((element) => element.isActive === true);
	let CAMconnectors = CAM.connectors.filter(
		(element) => element.isActive === true
	);

	// every concept should include text
	let CAMnodesNoText = CAMnodes.filter(
	...
	if (CAMnodes.length < config.ConNumNodes) {
        ...
        */
		...
		addElementsCy();
		let ResbfsAl = bfsAlgorithm("#" + cy.nodes()[0].id());
...
function saveCAMsuccess() {
	toastr.success(languageFileOut.popSave_01savedData, configToastr);
