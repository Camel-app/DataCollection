...
let hideResearcherButtonsTopUploadJSONButton = document.getElementById(
	"hideResearcherButtonsTop"
);
hideResearcherButtonsTopUploadJSONButton.innerHTML += uploadJSONButton;
...
$(function () {
	$("#fileToLoad").on("change", async (event) => {
		...
		/* parse to JSON file */
		let jsonObj = await fileToJSON(fileToLoad);
		...
		for (let i = 0; i < jsonObj.nodes.length; i++) {
			let elementNode = jsonObj.nodes[i];
		...
		// draw connectors
		for (let i = 0; i < jsonObj.connectors.length; i++) {
			//CAM.nodes.match(elt => elt.id ===     jsonObj.connectors[0].source)
			let elementConnector = jsonObj.connectors[i];
			//console.log(elementConnector);

			if (elementConnector.isActive) {
				let connector1 = new ConnectorCAM();
